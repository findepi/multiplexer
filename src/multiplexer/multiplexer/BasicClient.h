...
namespace multiplexer {
	...
	struct MessagesBufferTraits : public Base::MessagesBufferTraits {
	    ...
	    typedef azlib::SecondFromPairExtractor<value_type> ToRawMessagePointerConverter;
	    struct ToBufferRepresentationConverter : public std::unary_function<boost::shared_ptr<const RawMessage>, temporary_value_type> {
		temporary_value_type operator() (boost::shared_ptr<const RawMessage> raw) const {
		    return temporary_value_type(temporary_value_type::first_type(new boost::tribool(boost::logic::indeterminate)), raw);
		}
	    };
	    typedef azlib::FirstFromPairExtractor<temporary_value_type> SchedulingResultFunctor;
		...
		void notify_success(ConnectionsManagerImplementationWeakPointer, QueueType& qe) const {
		    AZDEBUG_MSG("notify_success called with " << qe.first.lock().get());
		...
		void notify_error(ConnectionsManagerImplementationWeakPointer, QueueType& qe) const {
		    AZDEBUG_MSG("notify_error called with " << qe.first.lock().get());
		    if (temporary_value_type::first_type tbp = qe.first.lock())
			*tbp = false;
		}
	    };
	 ...
	 *
	 * virtual what() is provided so that they are always distinguishable types (even for Boost.Python which translates them to Python exceptions)
	 */
	struct MxClientError : public azlib::Exception { const char* what() const throw() { return azlib::Exception::what(); } };
	struct NotConnected : public MxClientError { const char* what() const throw() { return MxClientError::what(); } };
	struct OperationTimedOut : public MxClientError { const char* what() const throw() { return MxClientError::what(); } };
	struct OperationFailed : public MxClientError { const char* what() const throw() { return MxClientError::what(); } };
    };
     ...
     */
    class BasicClient : public ConnectionsManager<BasicClient>, public boost::enable_shared_from_this<BasicClient>,
	...
	typedef std::deque<
		azlib::triple<boost::shared_ptr<const RawMessage>, ConnectionWrapper, boost::shared_ptr<MultiplexerMessage> >
	...
	void handle_message(Connection::pointer conn, boost::shared_ptr<const RawMessage> raw, boost::shared_ptr<MultiplexerMessage> mxmsg);

	// connectivity
	void shutdown();
	ConnectionWrapper async_connect(const Endpoint& peer_endpoint);
	bool wait_for_connection(ConnectionWrapper connwrap, float timeout) const;
	...
	void inline wait_for_incoming_message(azlib::SimpleTimer& timer) const {
		...
		if (timer.expired())
		    ...
		    // TODO logger.warning << "wait_for_incoming_message() operation failed for unknown reason";
	...
	unsigned int schedule_all(boost::shared_ptr<const RawMessage> raw) {
	    unsigned int c = 0;
	    for (ConnectionById::const_iterator i = connection_by_id_.begin(); i != connection_by_id_.end(); ++ i) {
	...
	BasicScheduledMessageTracker schedule_one(boost::shared_ptr<const RawMessage> raw) {
	...
	BasicScheduledMessageTracker schedule_one(boost::shared_ptr<const RawMessage> raw, ConnectionWrapper w, float timeout) {
	    Connection::pointer conn;
	    if ((conn = w.lock()) && conn->living()) {
		// TODO if we call schedule_one after long time of inactivity,
		// the `conn' may be still not notified about sockets having been closed by the remote peer.
		// This way, we may take this path of execution no knowing it will fail inevitably.
	...
	void inline handle_orphaned_outgoing_messages(MessagesBuffer& outgoing_messages) {
		...
		for (size_t n = working_connections.size(); n; --n) {
			...
			working_connections.splice(working_connections.end(), working_connections, working_connections.begin()); // move to the end
			message.first.reset();
		    } else
